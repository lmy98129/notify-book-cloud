...
module.exports = {
  possibleKnow: async (start, pageLength, customedRequestArray) => {
    try {
      let curUserProfile = await profile.check();
      let requestArray = [], recList = [], total = 0,
        arrayNames = ["jobArray", "contactArray", "degreeArray"];
      if (customedRequestArray !== undefined) {
        requestArray = customedRequestArray;
      } else {
